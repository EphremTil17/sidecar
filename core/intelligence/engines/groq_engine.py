...
class GroqEngine(BaseEngine):
    ...
    def stream_analysis(
        ...
        if not self.api_key:
            yield SidecarEvent(SidecarEventType.ERROR, content="Groq API Key is missing.")
            return
        ...
        if context_images:
            if len(context_images) > max_context_images:
                yield SidecarEvent(
                    ...
                    content=f"Groq Limit: Capping vault to most recent {max_context_images} images.",
                )
    ...
    def _execute_chat_completion(
        ...
        try:
            ...
            error_msg = f"Groq API Error: {e!s}"
    ...
    def stream_pivot(
        ...
        yield SidecarEvent(
            ...
            content=f"Pivot acknowledged. System re-tasked to {skill_data['identity'][:20]}...",
        )
        yield SidecarEvent(SidecarEventType.FINISH)
