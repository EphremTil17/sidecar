...
class AudioSensor:
    ...
    def start(self):
        ...
        try:
            ...
            self.actual_sample_rate = self.sample_rate
            logger.success(
                f"Audio Stream Active: [{sd.default.device[0]}] {sd.query_devices(sd.default.device[0], 'input')['name']} | WASAPI ({self.sample_rate}Hz)"
            )
        except Exception:
            ...
            try:
                ...
                logger.warning(
                    f"Audio Hardware rejected {self.sample_rate}Hz. Using native: {native_rate}Hz."
                )
                logger.success(
                    f"Audio Stream Active: [{sd.default.device[0]}] {device_info['name']} | WASAPI ({native_rate}Hz)"
                )
