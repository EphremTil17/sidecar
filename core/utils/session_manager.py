...
class SessionManager:
    ...
    def bootstrap(self) -> dict:
        ...
        CLI.print_welcome(
            self._state["monitor_index"],
            self._state["skill_name"],
            self.brain.get_model_name(),
        )
        CLI.print_ready()
    ...
    def _attempt_fast_boot(self, cache: dict) -> bool:
        ...
        try:
            # Domain Validation
            if not self.hardware.validate_cache(cache):
                return False
            if not self.knowledge.validate_cache(cache.get("skill_name")):
                return False

            # Display Context
            print(
                f"{CLI.Fore.CYAN}[i] Restoring: {cache.get('skill_name')} | Mon:{cache.get('monitor_index')} | Engine:{cache.get('engine_name')}"
            )
            ...
            self.hardware.apply_settings(
                self._state["monitor_index"], self._state["audio_device_id"]
            )
    ...
    def _init_core_engines(self):
        ...
        self.brain = SidecarBrain(
            settings.GOOGLE_API_KEY, settings.GROQ_API_KEY, self.skill_manager
        )
    ...
    def commit(self, overlay_geometry=None):
        ...
        if self.brain:
            ...
            if self.brain.current_skill_data:
                self._state["skill_name"] = self.brain.current_skill_data.get(
                    "name", self._state["skill_name"]
                )
