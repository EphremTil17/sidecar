...
class CLIFeedbackHandler:
    ...
    def handle_status_update(self, status):
        ...
        if "READY" in status:
            self.stop_ticker()
            # Combine the transition message with the READY block
            CLI.print_ready(pre_status=self._last_completion_msg)
            self._last_completion_msg = None
        elif "Handshake with" in status:
            # Start ticker but DON'T log yet to avoid double entries
            self.start_ticker(status.replace('Handshake with ', ''))
        elif "Thinking" in status:
            self._is_thinking = True
        elif any(k in status for k in ["Capturing", "Analyzing", "RECORDING", "Intent", "Offloaded", "Response Streaming", "Complete"]):
            if "Intent" in status:
                ...
                if was_ticking and "Response" in status:
                    ...
                    self._last_completion_msg = f"\x1b[90m{t_str} | \x1b[0m\x1b[32mSUCCESS  \x1b[90m| \x1b[0m{status}"
                elif "Offloaded" in status:
                    logger.info(status)
                else:
                    logger.info(status)
            else:
