...
class TerminalGhostWindow(QMainWindow, TerminalActionMixin):
    ...
    def _setup_terminal(self, font_family, font_size):
        ...
        self.terminal.setStyleSheet("background: transparent; border: none; padding: 10px 0px 10px 10px;")
    ...
    def _apply_init_ghost_mode(self):
        try:
            hwnd = int(self.winId())
            if apply_ghost_mode(hwnd): logger.success("Ghost Protocol applied to terminal window.")
    ...
    def toggle_focus_mode(self):
        ...
        self._update_container_style()
        if self._focus_mode: self.status_dot.show()
        else: self.status_dot.hide()
    ...
    def _on_event(self, event, payload):
        if event == AppEvent.UI_FOCUS_TOGGLE: self.toggle_focus_mode()
        elif event == AppEvent.UI_FONT_SCALE: self.increase_font_size() if payload > 0 else self.decrease_font_size()
        elif event == AppEvent.UI_MOVE: self.move_relative(*payload)
        elif event == AppEvent.UI_SCROLL: self.scroll_down() if payload > 0 else self.scroll_up()
        elif event == AppEvent.AGENT_CHUNK_UPDATE: self.append_markdown(payload[0])
        elif event == AppEvent.AGENT_STATUS_UPDATE: self.show_hud_notification(payload)
        elif event == AppEvent.AGENT_HEARTBEAT: self.update()
